...
public class BorrarUsuario extends javax.swing.JFrame {
    ...
    public BorrarUsuario(Controlador controlador) {
        initComponents();
        setVisible(true);
    ...
    private void mostrarTablaNombres() throws NotificarError{
        String matriz[][]= new String [controlador.buscarUsuarios().size()][2];
        List <Usuario> usuarios= controlador.buscarUsuarios();
         
        int i = 0;
        if(!usuarios.isEmpty()){
             for(Usuario e: controlador.buscarUsuarios()){
            matriz[i][0]=e.getEmail();
            matriz[i][1]=e.getContrasenia();
    ...
    private void initComponents() {
        ...
        getContentPane().add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 90, 560, 210));

        jLabel2.setText("Usuario a borrar:");
        getContentPane().add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 320, 190, 50));
        getContentPane().add(nombreborrar, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 330, 220, 40));
    ...
    private void TablaNombreMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TablaNombreMouseClicked
        ...
        if(row>=0){
             ...
             email =TablaNombre.getValueAt(row,0).toString();
            if(email.equals("")){
                JOptionPane.showMessageDialog(null, "Debe seleccionar un Usuario.");
                setVisible(true);
            }else{
                nombreborrar.setText(email);
            }    
        }        
    ...
    private void btnBorrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBorrarActionPerformed
        if((confirmado==true)&&(!(nombreborrar.getText().equals("")))){
            try {
                Usuario usuarioABorrar= controlador.buscarUnUsuario(email);
                if(usuarioABorrar!=null){
                    controlador.BorrarUsuario(usuarioABorrar);
                    JOptionPane.showMessageDialog(null,"Se borro el Usuario "+usuarioABorrar.getEmail());
                    dispose();
